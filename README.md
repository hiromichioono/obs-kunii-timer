...
├── chat_feed.html            # OBS ブラウザソース用チャットフィード（最新3件スクロール）
├── chat_overlay.html         # OBS ブラウザソース用チャットオーバーレイ（ピン留め表示）
...
| `chat_feed.html`          | チャットフィード表示（左下） | キャンバスと同解像度（例: 1920 × 1080） |
| `chat_overlay.html`       | ピン留めチャット表示（左下） | キャンバスと同解像度（例: 1920 × 1080） |
...
| チャットフィード | `http://localhost:8080/chat_feed.html`     |
...
| 種類 | 内容 | 表示先 |
| ---- | ---- | ------ |
| 初コメ歓迎 | `👋 {名前}さん、コメントありがとう！` | サーバーログのみ（未表示） |
| 個人コメント数マイルストーン | 3・11（サッカー特別数）・5刻み（~19）・10刻み（20~99）・50刻み（100~）の節目で `🏆 {名前}さん、{N}コメ達成！` | サーバーログのみ（未表示） |
